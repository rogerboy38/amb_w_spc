...
def quick_complete_operation(work_order, sequence, actual_time=None, measurements=None):
    ...
    try:
        ...
        return {'status': 'error', 'message': str(e)}
# Warehouse Management Jinja Template Functions
# These are referenced in hooks.py for template rendering

def get_warehouse_zones(warehouse=None):
    """
    Get warehouse zones for Jinja templates
    """
    try:
        if frappe.db.exists('DocType', 'Warehouse Zone'):
            if warehouse:
                return frappe.get_all('Warehouse Zone', 
                    filters={'warehouse': warehouse},
                    fields=['name', 'zone_name', 'zone_type', 'capacity', 'current_stock']
                )
            else:
                return frappe.get_all('Warehouse Zone', 
                    fields=['name', 'warehouse', 'zone_name', 'zone_type', 'capacity', 'current_stock']
                )
        return []
    except Exception:
        return []

def get_pick_task_status(pick_task=None):
    """
    Get pick task status for Jinja templates
    """
    try:
        if frappe.db.exists('DocType', 'Warehouse Pick Task'):
            if pick_task:
                status = frappe.db.get_value('Warehouse Pick Task', pick_task, 'status')
...
def get_material_assessment_status(material_assessment=None):
    """
    Get material assessment status for Jinja templates
    """
    try:
        if frappe.db.exists('DocType', 'Material Assessment Log'):
            if material_assessment:
                status = frappe.db.get_value('Material Assessment Log', material_assessment, 'status')
                return status or 'Draft'
            
            # Return status counts
            status_data = frappe.db.sql("""
                SELECT status, COUNT(*) as count
                FROM `tabMaterial Assessment Log` 
                GROUP BY status
            """, as_dict=True)
            return status_data
        return []
...
def get_warehouse_capacity(warehouse=None):
    """
    Get warehouse capacity for Jinja templates
    """
    try:
        if warehouse:
            # Get specific warehouse capacity
            wh_data = frappe.db.get_value('Warehouse', warehouse, 
                ['custom_total_capacity', 'custom_used_capacity', 'custom_available_capacity'], 
                as_dict=True
            )
            return wh_data or {}
        else:
            # Get all warehouses capacity summary
            warehouses = frappe.get_all('Warehouse', 
                fields=['name', 'warehouse_name', 'custom_total_capacity', 'custom_used_capacity']
            )
            capacity_data = []
            for wh in warehouses:
                total = wh.get('custom_total_capacity') or 0
                used = wh.get('custom_used_capacity') or 0

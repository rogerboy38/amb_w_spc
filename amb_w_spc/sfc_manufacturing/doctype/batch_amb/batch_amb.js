...
// Source: amb_w_tds production batch_amb.js (2026-02-14)
// Adapted for: amb_w_spc
// Version: BUG-102-FIX
// =============================================================================
...
frappe.ui.form.on('Batch AMB', {
    ...
    refresh: function(frm) {
        ...
        if (!frm.is_new()) {
            frm.add_custom_button(__("New Sample Request"), () => {
                frappe.call({
                    method: "amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.make_sample_request",
                    freeze: true,
    ...
    work_order: function(frm) {
        if (frm.doc.work_order) {
            frappe.call({
                method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.get_work_order_details',
...
function add_serial_tracking_buttons(frm) {
    ...
    if (!frm.doc.custom_serial_tracking_integrated) {
        frm.add_custom_button(__('🔗 Integrate Serial Tracking'), function() {
            frappe.call({
                method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.integrate_serial_tracking',
...
function generateSerialNumbers(frm) {
    ...
    if (isLevel4) {
        ...
        frappe.prompt([
            ...
            frappe.call({
                method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.generate_serial_numbers',
                args: {
...
function add_debug_button(frm) {
    frm.add_custom_button(__('🐛 Debug Test'), function() {
        frappe.call({
            method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.fixed_generate_serial_numbers',
            args: {
                batch_name: frm.doc.name,
                quantity: 1
            },
...
function setup_custom_buttons(frm) {
    ...
    frm.add_custom_button(__('Calculate Cost'), function() {
        frappe.call({
            method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.calculate_batch_cost',
            args: { batch_name: frm.doc.name },
            callback: function(r) {
                if (r.message) {
    ...
    frm.add_custom_button(__('Duplicate Batch'), function() {
        frappe.call({
            method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.duplicate_batch',
    ...
    frm.add_custom_button(__('Generate MRP'), function() {
        frappe.call({
            method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.calculate_material_requirements',
            args: {batch_name: frm.doc.name},
            callback: function(r) {
    ...
    frm.add_custom_button(__('Create Work Order'), function() {
        frappe.call({
            method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.create_work_order_from_batch',
            args: { batch_name: frm.doc.name },
            callback: function(r) {
                if (r.message && r.message.success) {
                    frappe.msgprint({
    ...
    frm.add_custom_button(__('Assign Golden Number'), function() {
        frappe.call({
            method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.assign_golden_number_to_batch',
            args: { batch_name: frm.doc.name },
            callback: function(r) {
                if (r.message && r.message.success) {
                    frappe.msgprint({
    ...
    frm.add_custom_button(__('Update Planned Qty'), function() {
        frappe.call({
            method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.update_planned_qty_from_work_order',
            args: { batch_name: frm.doc.name },
            callback: function(r) {
                if (r.message && r.message.success) {
                    frappe.msgprint({
    ...
    frm.add_custom_button(__('Sample Request'), function() {
        frappe.call({
            method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.create_sample_request',
            args: { batch_name: frm.doc.name },
            callback: function(r) {
                if (r.message && r.message.success) {
                    frappe.show_alert({
...
function add_bom_button_to_form(frm) {
    ...
    frappe.call({
        method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.check_bom_exists',
        args: { batch_name: frm.doc.name },
        callback: function(r) {
...
function generate_batch_code(frm) {
    frappe.call({
        method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.generate_batch_code',
        args: {
...
function load_batch_announcements(frm) {
    ...
    frappe.call({
        method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.get_running_batch_announcements',
        args: {},
        callback: function(r) {
            if (r.message && r.message.success) {
...
function fetch_work_order_data(frm) {
    ...
    if (frm.doc.work_order_ref) {
        frappe.call({
            method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.get_work_order_data',
...
function open_bom_creation_wizard(frm) {
    ...
    frappe.call({
        method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.get_packaging_from_sales_order',
        args: { batch_name: frm.doc.name },
        callback: function(r) {
            var pkg = r.message || {};
...
function create_bom_with_options(frm, options, dialog) {
    frappe.call({
        method: 'amb_w_spc.amb_w_spc.sfc_manufacturing.doctype.batch_amb.batch_amb.create_bom_with_wizard',
        args: {
            batch_name: frm.doc.name,
            options: options
        },
...
// =============================================================================
// END OF BATCH AMB CLIENT SCRIPT
// Adapted from amb_w_tds for amb_w_spc
// Version: BUG-102-FIX
// =============================================================================

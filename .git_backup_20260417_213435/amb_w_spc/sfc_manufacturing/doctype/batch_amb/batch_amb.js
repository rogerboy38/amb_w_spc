    ...
    const dialog = new frappe.ui.Dialog({
        ...
        fields: [
            ...
            },
            {
                ...
                change: function() {
                    // Auto-resolve prefix from packaging type
                    ...
                    if (pkg) {
                        frappe.db.get_value('Item', pkg, ['item_name', 'weight_per_unit', 'weight_uom'], function(r) {
                            if (r) {
                                ...
                                let prefix = 'BRL';
                                if (name_lower.includes('sample') || name_lower.includes('bolsa') || name_lower.includes('bag')) {
                                    prefix = 'SMP';
                                } else if (name_lower.includes('ibc') || name_lower.includes('tote')) {
                                    prefix = 'IBC';
                                } else if (name_lower.includes('drum') || name_lower.includes('cunete') || name_lower.includes('cuete') || name_lower.includes('pail')) {
                                    prefix = 'CTE';
                                }
                                ...
                                dialog.fields_dict.tara_weight.set_description(
                                    __('From Item: {0} ({1} {2})', [r.item_name, r.weight_per_unit || 0, r.weight_uom || 'Kg'])
                                );
                            }
                        });
                    }
        ...
        primary_action: function() {
            ...
            generate_serials_l2(frm, data.barrel_count, data.prefix, data.packaging_type, data.tara_weight);
            dialog.hide();
        }
    });
    dialog.show();
